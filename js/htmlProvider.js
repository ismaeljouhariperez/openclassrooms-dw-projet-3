class HtmlProvider
{
    ...
    static closeButton() // Create close button on form
    {
    ...
    static secondForm()
    {
         ...
         let $inputSignature = $('<p>Pour finaliser votre réservation, veuillez signer ci-dessous</p>')
        
         let $divOptions = $('<div id="options"></div>');
         let $previous = $('<img src="images/icons/previous-icon.svg" id="previousForm" />');
         ...
         let $canvas = $('<canvas id="canvasSignature" class="canvas" width="250px" height="160px"></canvas><span id="errorSignature"></span>')

         let $secondForm = [];
         $secondForm.push($bookForm, $textSurname, $inputSurname, $textName, $inputName, $inputSignature, $canvas, $divErase, $iconErase, $textErase, $divOptions, $previous, $next);
         return $secondForm;
    }
    ...
    static thirdForm()
    {
        ...
        let $textSuccess = $("<p> Merci d'avoir réservé avec Velo Luxembourg.</p><p>Votre vélo est réservé pour les 20 prochaines minutes à la station " + $station.substr(7) + ", ne perdez pas de temps !</p><p>À très bientôt sur le réseau Velo Luxembourg.</p><p>En sélectionnant une nouvelle station, votre réservation sera annulée. </p> ");
    ...
    static dialogModal()
    {
        ...
        let $text = $("<p>En cliquant sur le bouton Annuler, votre réservation actuelle sera annulée.</p><p>Vous pourrez effectuer une nouvelle réservation à la station sélectionnée.</p>")
        ...
        let $continueBtn = $('<button type="button" id="continue" class="mdl-button">Annuler</button>')
