class Canvas
{
    ...
    touchStart(e)
    {
        ...
        this.isDrawing = true;
        this.context.beginPath();
        this.context.moveTo(e.touches[0].clientX - parseInt(this.rect.left), e.touches[0].clientY - parseInt(this.rect.top));
    }
    ...
    touchMove(e)
    {
        if (this.isDrawing)
        {
            ...
            this.context.lineTo(e.touches[0].clientX - this.rect.left, e.touches[0].clientY - this.rect.top);
            this.context.stroke();
        }
    }
    ...
    { 
        ...
        let count = 0;
        for (let i = 0; i < this.imageData.data.length; i+=4) // Check every pixel
        {
            if (this.imageData.data[i+3] != 0) { count++; } // Check if opacity value change
        }
        return count > 200;
    }
    ...
    canvasActions() // Adds a visual confirmation or alert to verify if the user has correctly signed
    {
        ...
        else
        {
            ...
            $('#errorSignature').show();
            $('#errorSignature').html("<p>*Votre signature n'est pas assez longue</p>")
        }
    }
