class LeafletMap
{
    ...
    displayLayer() // Load and display tile layer on the map
    {
    ...
    centerMap()
    {
        document.getElementById('setView').addEventListener('click', () => this.map.flyTo(L.latLng(49.610821, 6.132882), 14))
    }
    ...
    // Change marker icon regarding station status
    markerIcon(available, status)
    {
        if (available == 0) { return L.icon({iconUrl: 'images/icons/marker-nonactive-icon.svg', id:'markerIcon'})}
        else if (available > 0) { return L.icon({iconUrl: 'images/icons/marker-open-icon.svg', id:'markerIcon'})}
        else if (status === 'CLOSE') { return L.icon({iconUrl: 'images/icons/marker-close-icon.svg', id:'markerIcon'})}
    }
    ...
    initMarkers()
    {
        ...
        {
            console.log("Liste des stations JCDecaux à Luxembourg", data); // Expected output : 92 objects
            for (let station of data) // station = data[i]
            {
                let stations = [station.position.lat, station.position.lng];                
                let marker = L.marker(stations,{icon:this.markerIcon(station.available_bikes, station.status)}).addTo(this.map);
                ...
                marker.on('click', (e) =>
                {
                    if (station.available_bikes != 0)
                    {
                        ...
                        $('#stationPlaces').html("<p>" + '<img src="images/icons/parking-icon.svg" alt="Parking icon"/>' + "   "  + station.bike_stands + " places disponibles </p>");
                        $('#stationAvailability').html("<p>" + '<img src="images/icons/bike-icon.svg" alt="Bike icon"/>' + "   " + station.available_bikes + " vélos disponibles </p>");
                        $('#stationNumber').html("<strong>" + '<img src="images/icons/marker-icon.svg" alt="Marker icon"/>'+ ' Station n°' + station.number + "</strong>");
                        // Display popup
                        marker.bindPopup(station.name.substr(7)).openPopup();
                        L.DomUtil.addClass(marker._icon, "blinking"); // Add animation
                    }
                    else // Prevent from clicking on a unavailable station
                    {
    ...
    async fetchData()
    {
        try
        {
            let dataURL = 'https://api.jcdecaux.com/vls/v1/stations?contract=Luxembourg&apiKey=28b0fb05678c8dbbccf28ee6cd3b1771fc560c43';
