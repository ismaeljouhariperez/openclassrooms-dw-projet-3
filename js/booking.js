class Booking
{
    ...
    displayForm() // Display user reservation form
    {
        ...
        // Append and display close Button
    ...
    bookAction() // Action on clicking submit button
    {
    ...
    openDialog() // Open a dialog window if a reservation is in progress
    {
        if (this.open === false) // Prevent from opening multiple dialog windows each time a user clicks on submit
        {
            ...
            {
                // Remove previous station position and name in session Storage
                sessionStorage.removeItem('position');
                sessionStorage.removeItem('name'); 
                $('dialog').fadeOut('slow');
                $('dialog').remove();
                this.clearTimerInterval(); // Clear previous reservation
                this.removeTimerModal();
                this.book(); // Launch new reservation
            })
    ...
    {   
        ...
        for (let k = 10 ; k <= 12; k++) { secondForm[10].append(secondForm[k]) }
        ...
        this.canvas = new Canvas(document.getElementById('canvasSignature'), 'gold', 1)
    ...
    previous() // Previous button action
    {
        $('#bookForm').remove();    
        $('#options').remove();                    
    ...
    checkInput(elt) // Check input elements adding a visual reminder for the user
    {
    ...
    inputListener() // Listen input contents
    {
    ...
    setTimerInterval()
    {
        setInterval(() =>
        {
            ...
            if (seconds == 0 && minutes == 0) // Stop timer and remove modal when finished
            { 
                this.cancelBooking();
            }
        }, 1000)
    ...
    cancelBooking() // Cancel the current reservation
    {
    ...
    findMyStation() // Locate last reservation
    {
        // Create an array containing lat and lng as strings
    ...
    appendTimerModal() // Append modal when Booking is valid
    {
        if (sessionStorage.getItem('timestamp') != null)
        {
    ...
    removeTimerModal() // Display modal is the user cancels
    {
    ...
    validationForm() 
    {
        $('#successMessage').remove();
        $('#bookForm').hide(); 
        $('#options').remove(); // Remove previous forms
        // Append last form
    ...
    next() // Actions when clicking next button
    {
        // Check if the user has filled correctly all the requested fields
        if (this.checkInput(this.name) && this.checkInput(this.surname) && this.canvas.pixelChecker())
        {
        ...
        else if (this.inputValidation == false) // Prevent from going further
        {
